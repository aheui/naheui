...
module.exports = class Machine {
    ...
    constructor(codeSpace) {
        ...
        this._cursor = new Cursor(0, 0, 0, 1);

        this._selectedStorage = 0;
        this._storage = store[this._selectedStorage];
    }
    ...
    run(terminateFunction) {
        ...
        if (!!terminateFunction) {
            const result = this._storage.pop() | 0;
     ...
     */
    step() {
        const code = this._cursor.point(this.codeSpace);

        if (!!code) {
            // cursor turn result
            this._cursor.turn(code.jung);
            const operation = operationMap[character.cho[code.cho]];

            if (!!operation) {
                if (this._storage.count() < count.parameter[code.cho]) {
                    // cursor reflect result
                    this._cursor.reflect();
                } else {
                    this._terminated = operation(this, code.jong);
                }
            }
        }
        ...
        if (!this._terminated) {
            // cursor move result
            this._cursor.move(this.codeSpace);
        }
    }
     ...
     */
    getStorage(code) {
        return this._storage[indexing.jong(code)];
    }
     ...
     */
    selectStorage(code) {
        this._selectedStorage = indexing.jong(code);
        this._storage = this.getStorage(this._selectedStorage);
    }
    ...
    dump(format) {
        ...
        function classicStyleDump(lang) {
            const code = _cursor.point(codeSpace);
            ...
            return [
                `${ coorMsg }: (${ _cursor.x }, ${ _cursor.y })`,
                `${ charMsg }: ${ code || '' }`,
            ]
            .concat(store.map((_storageTemp, index) => [
                (_storage === _storageTemp) ? '>' : '',
                String.fromCharCode('아'.charCodeAt() + index),
                ': ',
                _storageTemp.dump('csv reverse'),
            ]
            .join('')))
            .join('\n') + '\n';
        }
